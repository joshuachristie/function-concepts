...
** 20200205 notes from linguistic passages sent by Monika
  ...
  - elements other than the head in a nominal phrase are /modifiers/ (/premodifiers/ if they precede the head; /postmodifiers/ if they follow the head). You can have multiple pre/post modifiers. If a premodifier is interuppted by the head, then it is a /discontinuous modifier/
...
*** noun phrases 
I'll first consider when the base form "function" is used.
*Case 1*: function is the subject of the sentence or the head noun of a noun phrase that acts as the subject.
Examples: "The function of the gene is to ...", "The function is to...", "The important function of the gene is...", "Protein function is important for..."
These are straightforward cases, which match the "standalone form" that I had previously described them as (this terminology needs to be changed)
*Case 2*: 


in the base form "function", which case it 
