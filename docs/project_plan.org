...
In many instances, *Case 1* can be directly unpacked. When function is the head noun of a noun phrase that acts as the subject, it requires a little unpacking.
...
These can be converted into the noun "function"

*Case 3*, functional as premodifier of noun phrase used as subject
If concrete, unpack as "<concrete noun> functions to"
If abstract and related to activity, importance, role, etc. (e.g. "functional role"), throw away abstract head noun and try to unpack as "function"
*Case 4* functional used in a predicative manner as lone adjective (as subject attribute).
...
*Case 5* (look at cases where head is abstract and when concrete)
