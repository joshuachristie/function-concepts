...
but if we have functional in the sense of "this is its function" then it couldn't be intensified so this could be one way of telling them apart perhaps?
...
this seems to be adjective (functional) -> noun (by adding ity)

I'm going to assume that Case 2 is impossible.
In many instances, Case 1 can be directly unpacked. When function si the head noun of a noun phrase that acts as the subject, it requires a little unpacking.
...
I will now try to identify other cases that can be unpacked with the fewest modifications.
