...
1. 
   ...
   - Now we try to identify /y/ by asking "what do 'signaling cues and enzymes' do?" and getting the answer "regulate individual sites" where "regulate" is a transitive verb taking the object "individual sites".
...
So at least for this example, it does hold that when function is the subject of the subordinate phrase you do (or at least can) identify /y/. 
...
I can split it into two components: (i) dissecting sentence structure; (ii) getting to a 3.7 equivalent in which I can split off into step 4 or 5.
...
2. Identify /x/ and /y/
   - Is function the subject (or part of a noun phrase that is the subject)? If function itself is the subject (or head of noun phrase), then look at its modifiers, complements or dependents for /x/. For /y/, look to the transitive verb of the subordinate clause and its object
   - If function is the verb, then the subject of the subordinate phrase is /x/ and the (e.g. "the gene that functions as a regulator" -> "that functions as a regulator" (adjective phrase with subject "the gene", verb "functions", "as a regulator" as an adverb phrase that modifies the verb functions)
   - If function is the predicate, then the subject of the subordinate phrase is /x/ but you shouldn't be able to identify /y/. It is important to differentiate function used as a subject complement (e.g. "

(what about the example "the gene that has a function", subphrase is "that has a function" and so function is in the predicative form; however has indicates a state of identity/possession/essence and so I think we should treat it as a linking verb linking the subject to its subject complement (clearly "a gene having a function" is part of its essence, as opposed to "a boy having an icecream".
3. 
4. Starting from function, or the clause it is part of, process each part of speech working outwards. (First modifiers of phrase containing function then complements/dependents (function might have complements/dependents or it might be a complement/dependent itself))

Step 3.1 definitely needs to be redone as it contains vague, linguistically incorrect instructions.

make a note about treating any verb that indicates essence/identity as a linking verb and its object as a subject complement.
